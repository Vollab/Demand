...
class VacancyModel {
	...
	async insert(candidate: Omit<Vacancy, 'id' | 'open' | 'updated_at' | 'created_at'>) {
		const { demand_id, activity_area_id, name, description, state, city, street, work_mode } = candidate

		return this.db.query<Vacancy>(
			`
			INSERT INTO
				demand.vacancy (demand_id, activity_area_id, name, description, state, city, street, work_mode, open)
			VALUES
				($1, $2, $3 ,$4, $5, $6, $7, $8, $9)
			RETURNING
				*
	...
	async update(id: Vacancy['id'], candidate: PartialOmit<Vacancy, 'id' | 'updated_at' | 'created_at'>) {
